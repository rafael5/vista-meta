...
# .env is optional — there is no required configuration. Sourced only so
# a user CAN override defaults (e.g. HOST_BIND_IP) if they want to expose
# the engine beyond localhost. Missing .env is not an error.
-include .env

# Host bind address for published ports. Localhost-only by default; no
# setup required. Override via .env or environment to expose elsewhere.
HOST_BIND_IP ?= 127.0.0.1
export HOST_BIND_IP   # reach the `docker compose` child process
...
# still use plain docker by container name for terseness. compose.yml
# defaults HOST_BIND_IP to 127.0.0.1, so no env file is needed; if the
# user keeps a root .env to override it, we pass it through explicitly
# (compose's default lookup is next to the compose file, one level down,
# which would miss a .env at repo root).
COMPOSE_CMD := $(DOCKER) compose $(if $(wildcard .env),--env-file .env,) -f $(COMPOSE) -p $(PROJECT)
...
shell: ## SSH into the container as vehu
	ssh -p 2222 -o StrictHostKeyChecking=no vehu@$(HOST_BIND_IP)
