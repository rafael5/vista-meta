...
|---|---|---|
| **[VistA Compass](vscode-extension/)** (in-repo, 0.2.x) | 5 code-model + 2 data-model TSVs | VSCode situational awareness while reading M code: sidebar (tags, callers, callees, globals, XINDEX) + hovers that join a `^GLOBAL` token → FileMan file → PIKS category |
| **[vista-compass](https://github.com/vista-forge/vista-compass)** | the published `data-v1` release | the successor VSCode extension over the *released* model (twin of vista-atlas, which covers the vdocs corpus). Supersedes the retired vista-info-hub navigator |
| **MCP server** (in-repo, [`host/scripts/mcp_server.py`](host/scripts/mcp_server.py)) | the `meta.db` projection of all 24 TSVs | query/lookup/bridge/orientation tools over stdio JSON-RPC — the AI-agent front door, twin of vdocs `serve-mcp`. The `package-namespace.tsv` `app_code`/`vdl_id` columns anchor the vdocs entity bridge |
